 ...
 */
class TCPClientPort : public Port, protected StoppableThread
{
  NarrowString<16> host;
  unsigned port;
...
public:
  /**
   * Creates a new TCPClientPort object, but does not open it yet.
   *
   * @param host an IP4 address (e.g. 127.0.0.1)
   * @param port the port number (1..32767)
   * @param handler the callback object for input received on the
   * port
   */
  TCPClientPort(const char *host, unsigned port, Handler &handler);
